...
def preprocess_prediciton(iq):
    ...
    """
    df = df.drop(columns={'open', 'min', 'max', 'avg_gain',
                 'avg_loss', 'L14', 'H14', 'gain', 'loss'})
...
while (1):
    ...
    # LOGO, ELE PRECISA DE TEMPO PRA ELABORAR A PREVISÃO ANTES DE ATINGIR OS 59 SEGUNDOS PRA ELE
    pred_ready = preprocess_prediciton(iq)
    # FAZER A APOSTA, ENÃO ELE VAI TENTAR PREVER O VALOR DA TERCEIRA NA FRENTE
    ...
    if trade:
        ...
        # print(datetime.datetime.now().second)
        print("Checking if win or lose")
        betsies, result = iq.check_win_v4(id)
        """ response with ('loose', -1.0) """
        ...
        """ check if win to append to bets list """
        bets.append(betsies)
